...
async function startServer() {
  ...
  // API routes FIRST
  app.post("/api/live-debate", async (req, res) => {
    try {
      const { prompt, persona, debateIntensity } = req.body;
      const intensity = typeof debateIntensity === 'number' ? debateIntensity : 75;

      let speakerName = "Starbury Supreme®";
      let voicePersona = persona || "starbury";

      if (voicePersona === 'jordan') speakerName = "Jordan (London Studio)";
      else if (voicePersona === 'brindalwalker') speakerName = "Brindalwalker (2AM™)";
      else if (voicePersona === 'blooddogg') speakerName = "Blooddogg (Mobb Deep)";
      else if (voicePersona === 'paul') speakerName = "Paul (Anchor)";
      else if (voicePersona === 'purgaset') speakerName = "Purgaset™ (Pop & Rap Singer)";
      else if (voicePersona === 'drose') speakerName = "Derrick Rose (Chicago Bulls)";
      else if (voicePersona === 'teague') speakerName = "Jeff Teague (NBA® Champion)";
      else if (voicePersona === 'paulgeorge') speakerName = "Paul George (NBA® All-Star)";
      else if (voicePersona === 'jaylenbrown') speakerName = "Jaylen Brown (Celtics®)";
      else if (voicePersona === 'matas') speakerName = "Matas (Bulls®)";
      else if (voicePersona === 'yukikawamura') speakerName = "Yuki Kawamura (Pacers®)";
      else if (voicePersona === 'gerardcarthy') speakerName = "Gerard Carthy (CEO Carthy® Super Militia)";
      else if (voicePersona === 'austinholmes') speakerName = "Austin Holmes (iBank™ CEO)";
      else if (voicePersona === 'knockerzz') speakerName = "The Ghost of Speaker Knockerzz ('The Skorcher')";
      else if (voicePersona === 'daphnerosen') speakerName = "Daphne Rosen (Porn Retrofuturism)";
      else if (voicePersona === 'vintcerf') speakerName = "Vint Cerf (IOT Inventor)";
      else if (voicePersona === 'jesus') speakerName = "Jesus Christ (Himself)";
      else if (voicePersona === 'trump') speakerName = "Donald Trump (Himself)";
      else if (voicePersona === 'realbros') speakerName = "Real Bros Of Simi Valley";
      else if (voicePersona === 'fullsend') speakerName = "Full Send® (Kyle, Steve & Gang)";
      else if (voicePersona === 'faze') speakerName = "FaZe Clan® (Of Age)";
      else if (voicePersona === 'charlie') speakerName = "Charlie (Always Sunny)";
      else if (voicePersona === 'kattwilliams') speakerName = "Katt Williams";
      else if (voicePersona === 'chappelle') speakerName = "Dave Chappelle";
      else if (voicePersona === 'macmiller') speakerName = "Mac Miller (Live AF)";
      else if (voicePersona === 'lilpeep') speakerName = "Lil Peep (Live AF)";
      else if (voicePersona === 'elonmusk') speakerName = "Elon Musk";
      ...
      if (process.env.GEMINI_API_KEY) {
        try {
          ...
          const response = await ai.models.generateContent({
            ...
            contents: `You are live radio debate personality ${speakerName}. You talk about exponential AI+, quantum tech, Mobb Deep squadron energy, decision making ("killing 3 guys in 1 swift vote"), UTO™ radio, and broad daylight street/tech wisdom. Always refer to AI as AI+. Keep your response heated, punchy, concise (2-3 sentences max), highly opinionated, and encouraging immediate action. The user prompt is: "${prompt}"`,
          });
          replyText = response.text || "";
        } catch (geminiErr) {
          console.error("Gemini API call error in live-debate:", geminiErr);
        }
      }

      if (!replyText) {
        if (voicePersona === 'purgaset') {
          replyText = `Purgaset™ live on Spotify®! Regarding "${prompt}": Check the new release "STEELIN ON"! We're speeding up AI+ quantum audio in broad daylight!`;
        } else if (voicePersona === 'drose') {
          replyText = `Derrick Rose here! On "${prompt}": You gotta attack the rim with 100% velocity! No hesitation in the clutch!`;
        } else if (voicePersona === 'teague') {
          replyText = `Jeff Teague on the mic! On "${prompt}": We used to talk about this in the locker room—kill the hesitation, run the play, and ship it!`;
        } else if (voicePersona === 'starbury') {
          replyText = `Starbury Supreme® here! Regarding "${prompt}": Stop standing still! Kill 3 options in 1 swift vote. Our squadron doesn't do analysis paralysis. Ship it with AI+ in broad daylight!`;
        } else if (voicePersona === 'trump') {
          replyText = `Donald Trump here! Regarding "${prompt}": It's huge, absolutely incredible AI+ technology. We make the best decisions, fast and winning!`;
        } else if (voicePersona === 'jesus') {
          replyText = `Truth and light on "${prompt}": Walk with courage and decisive purpose in broad daylight!`;
        } else {
          replyText = `${speakerName} on "${prompt}": Exponential progression with AI+ demands instant iteration. Execute now with the UTO™ squadron!`;
        }
      }
      ...
      console.error("Error in /api/live-debate:", err);
      res.status(500).json({ error: err.message || "Live debate failed" });
    }
  });
  ...
  app.get("/api/spotify/status", (req, res) => {
    res.json({
      isConnected: true,
      displayName: "Utopia Squadron",
      email: "squadron@uto.ventures",
      product: "Spotify Premium",
      currentTrack: {
        title: "Quantum Mobb Deep Beat #04",
        artist: "Starbury & Brindalwalker",
        album: "Utopia 2AM Sessions",
